...
from advert.services.services import create_request_to_parts
...
async def add_description(message: types.Message, state=FSMContext):
    ...
    async with state.proxy() as data:
        await create_request_to_parts(mark_auto=data['mark_auto'], model_auto=data['model_auto'],
                                      year_auto=data['year_auto'],
                                      description=data['description'], pk=pk)
        await message.reply(f"""
        Вы создали запрос на запчасть для машины:
...
def register_handlers_add_request_to_part(dp: Dispatcher):
    dp.register_message_handler(add_request, text=['Запрос на запчасть'], state=None)
